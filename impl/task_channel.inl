...
inline task_t task_channel::enqueue_task(TaskHandler&& th, CompletionHandler&& ch, InsertIterator& it)
{
    ...
    if (opid.valid())
    {
        typedef detail::protected_t<std::remove_reference<TaskHandler>::type> TaskHandlerP;
        typedef detail::protected_t<std::remove_reference<CompletionHandler>::type> CompletionHandlerP;
        auto* wrapper = detail::create_task_handler(
            ...
            std::bind(&task_channel::completion_handler_proxy<CompletionHandlerP>,
                shared_from_this(),
                placeholders::error,
                detail::protected_t<std::remove_reference<CompletionHandler>::type>(std::forward<CompletionHandler>(ch))),
            opid);
